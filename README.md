...
**OMC Translator is not currently in operation**
...
| 21:00 + duration | Wait until contest ends | internal helper |
| end-time | Fetch & translate official editorials | `scripts/fetch_editorial.py` |
| end-time + α | Fetch & translate past 3 contests (tasks & editorials) | `scripts/update_past3.py` |
...
1. Create `languages/<lang>/static`.
...
このリポジトリは [Online Math Contest](https://onlinemathcontest.com)（OMC）の「問題文・公式解説・ユーザー解説」を自動取得し、多言語（現在英語のみ）へ翻訳して公開します。
数式は KaTeX を保持したまま GPT‐4o-mini で翻訳し、ブラウザで再レンダリングします。

**現在運用はされていません。**

毎日 GitHub Actions で主に以下のフローを実行します：

| 日本時間                 | 処理                            | スクリプト                       |
| -------------------- | ----------------------------- | --------------------------- |
| 20:00            | プログラム開始              | `participate_today.py`      |
| 21:00            | 問題文取得＋翻訳  | `fetch_and_translate.py`    |
| 終了時刻まで               | wait                          | 内部                          |
...
# 単一ファイルだけ再翻訳
python scripts/translate_specific_remote.py task omc001 A
```
